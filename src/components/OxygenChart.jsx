...
// Filter a été ajouté à l'import pour éviter le crash
import { Eye, Filter } from 'lucide-react';
import { ResponsiveContainer, LineChart, Line, XAxis, YAxis, Tooltip, CartesianGrid } from 'recharts';

export default function OxygenChart({ historique }) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800/60 pb-4">
        <div>
          <h3 className="text-lg font-black text-white tracking-tight flex items-center gap-2">
            <Eye size={18} className="text-pink-400" /> Historique Oxygène Dissous (O₂)
          </h3>
          <p className="text-xs text-slate-500 mt-0.5">Suivi de la respiration du milieu en mg/L</p>
        </div>
      ...
      <div className="h-80 w-full bg-slate-950/40 rounded-2xl border border-slate-800/50 p-4">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={donneesFiltrees}>
            ...
            <YAxis stroke="#475569" fontSize={10} tickLine={false} domain={[0, 15]} />
            <Tooltip contentStyle={{ backgroundColor: '#090d16', borderColor: '#1e293b', borderRadius: '12px' }} />
            <Line name="Oxygène (mg/L)" type="monotone" dataKey="oxygene" stroke="#ec4899" strokeWidth={2.5} dot={false} activeDot={{ r: 5 }} connectNulls />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
